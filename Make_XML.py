...
def get_image_data(path):
    ...
    return img_name ,img_width, img_height, num_hands

def generate_XML(name, width, height, num_hands, obj_names, obj_labels):
    annotation = ET.Element("annotation")
    add_folder = ET.SubElement(annotation,"folder")
    add_folder.text = " "
    add_filename = ET.SubElement(annotation,"filename")
    add_filename.text = os.path.basename(name)
    ...
    add_width.text = width
    add_height = ET.SubElement(add_size,"height")
    add_height.text = height
